...
LeafletWidget.methods.addDrawToolbar = function (
  ...
  (function () {
    ...
    if (targetLayerId) {
      ...
      editableFeatureGroup = map.layerManager.getLayer(
        "geojson",
        targetLayerId
      );
    ...
    if (options && options.draw && options.draw.marker) {
      if (
        ...
        options.draw.marker.icon = options.draw.marker.markerIconFunction(
          options.draw.marker.markerIcon
        );
      }
    }
    ...
    if (!$.isEmptyObject(options.edit)) {
      ...
      if (!options.edit.edit) {
        ...
        editOptions.edit = {};
        editOptions.edit.selectedPathOptions = options.edit.selectedPathOptions;
      }
    ...
    map.on(L.Draw.Event.DRAWSTART, function (e) {
      if (!HTMLWidgets.shinyMode) return;
      Shiny.onInputChange(map.id + "_draw_start", {
    ...
    map.on(L.Draw.Event.CREATED, function (e) {
      ...
      layer.on(
        "click",
        mouseHandler(map.id, featureId, targetGroup, layerCategory + "_click"),
        map
      );
      layer.on(
        "mouseover",
      ...
      layer.on(
        ...
        mouseHandler(
          ...
          featureId,
          targetGroup,
          layerCategory + "_mouseout"
        ),
        map
      );

      Shiny.onInputChange(map.id + "_draw_new_feature", layer.toGeoJSON());
      Shiny.onInputChange(
        map.id + "_draw_all_features",
    ...
    map.on(L.Draw.Event.EDITED, function (e) {
      ...
      Shiny.onInputChange(map.id + "_draw_edited_features", layers.toGeoJSON());
      Shiny.onInputChange(
        map.id + "_draw_all_features",
        editableFeatureGroup.toGeoJSON()
      );
...
LeafletWidget.methods.removeDrawToolbar = function (clearFeatures) {
  (function () {
    ...
    if (map._editableFeatureGroupName && clearFeatures) {
      var featureGroup = map.layerManager.getLayerGroup(
        map._editableFeatureGroupName,
        false
      );
...
LeafletWidget.methods.getDrawnItems = function () {
  ...
  if (map._editableGeoJSONLayerId) {
    featureGroup = map.layerManager.getLayer(
      "geojson",
      map._editableGeoJSONLayerId
    );
  } else if (map._editableFeatureGroupName) {
    featureGroup = map.layerManager.getLayerGroup(
      map._editableFeatureGroupName,
      false
    );
  }
